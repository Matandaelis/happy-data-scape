...
export function DashboardSidebar({ active, setActive }: { active: string; setActive: (k: string) => void }) {
  ...
  return (
    <aside className="fixed left-0 top-0 bottom-0 w-[228px] bg-sidebar border-r border-sidebar-border flex flex-col z-[100] overflow-hidden">
      {/* Logo */}
      <div className="px-[18px] h-14 flex items-center border-b border-sidebar-border shrink-0">
        <div>
      ...
      <nav className="flex-1 overflow-y-auto p-2 flex flex-col gap-0.5">
        {NAV.map((item) => {
          ...
          return (
            <div key={item.k}>
              <button
                onClick={() => (item.children ? toggle(item.k) : setActive(item.k))}
